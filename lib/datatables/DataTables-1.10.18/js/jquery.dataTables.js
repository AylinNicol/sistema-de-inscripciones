...
(function( $, window, document, undefined ) {
	...
	DataTable = function ( options )
	{
		 ...
		 *      oTable.fnFilter('Webkit');
		 *      data = oTable._('tr', {"search": "applied"});
		 *
		...
		this.fnAddData = function( data, redraw )
		{
			...
			rows = $.isArray(data) && ( $.isArray(data[0]) || $.isPlainObject(data[0]) ) ?
				api.rows.add( data ) :
				api.row.add( data );
		...
		{
			api = this.api( true ).columns.adjust();
		...
		this.fnGetData = function( src, col )
		{
			...
			if ( src !== undefined ) {
				type = src.nodeName ? src.nodeName.toLowerCase() : '';
		...
		{
			api = this.api( true ).page( mAction );
		...
		{
			api = this.api( true ).column( iCol ).visible( bShow );
		...
		for ( fn in DataTable.ext.internal ) {
			if ( fn ) {
		...
		this.each(function() {
			...
			o = {};
			oInit = len > 1 ? // optimisation for single table case
				_fnExtend( o, options, true ) :
				options;

			/*global oInit,_that,emptyInit*/
			i=0, iLen, j, jLen, k, kLen;
			...
			allSettings = DataTable.settings;
			for ( i=0, iLen=allSettings.length ; i<iLen ; i++ )
			{
				...
				if (
					...
					bRetrieve = oInit.bRetrieve !== undefined ? oInit.bRetrieve : defaults.bRetrieve;
			...
			oSettings = $.extend( true, {}, DataTable.models.oSettings, {
			...
			if ( oInit.iDeferLoading !== null )
			{
				...
				tmp = $.isArray( oInit.iDeferLoading );
				oSettings._iRecordsDisplay = tmp ? oInit.iDeferLoading[0] : oInit.iDeferLoading;
				oSettings._iRecordsTotal = tmp ? oInit.iDeferLoading[1] : oInit.iDeferLoading;
			}
			
			/* Language definitions */
			oLanguage = oSettings.oLanguage;
			...
			stripeClasses = oSettings.asStripeClasses;
			rowOne = $this.children('tbody').find('tr').eq(0);
			...
			loadedInit = function () {
				...
				captions = $this.children('caption').each( function () {
	...
	_ext; // DataTable.ext
	_Api; // DataTable.Api
	_api_register; // DataTable.Api.register
	_api_registerPlural; // DataTable.Api.registerPlural
	...
	_areAllUnique = function ( src ) {
		...
		sorted = src.slice().sort();
		last = sorted[0];
	
		for ( i=1, ien=sorted.length ; i<ien ; i++ ) {
			if ( sorted[i] === last ) {
				return false;
			}
	...
	function _fnLanguageCompat( lang )
	{
		...
		defaults = DataTable.defaults.oLanguage;
	...
	function _fnCompatOpts ( init )
	{
		...
		// element by element
		searchCols = init.aoSearchCols;
	
		if ( searchCols ) {
			for ( i=0, ien=searchCols.length ; i<ien ; i++ ) {
				if ( searchCols[i] ) {
	...
	function _fnAddColumn( oSettings, nTh )
	{
		...
		oDefaults = DataTable.defaults.column;
		...
		searchCols = oSettings.aoPreSearchCols;
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		if ( ! oCol.sWidthOrig ) {
			...
			// Style attribute
			t = (th.attr('style') || '').match(/width:\s*(\d+[pxem%]+)/);
			if ( t ) {
				oCol.sWidthOrig = t[1];
			}
		}
		...
		oCol.fnGetData = function (rowData, type, meta) {
			innerData = mData( rowData, type, undefined, meta );
		...
		bAsc = $.inArray('asc', oCol.asSorting) !== -1;
		bDesc = $.inArray('desc', oCol.asSorting) !== -1;
	...
	{
		aiVis = _fnGetColumns( oSettings, 'bVisible' );
	...
	function _fnApplyColumnDefs( oSettings, aoColDefs, aoCols, fn )
	{
		...
		if ( aoColDefs )
		{
			...
			for ( i=aoColDefs.length-1 ; i>=0 ; i-- )
			{
				...
				aTargets = def.targets !== undefined ?
					def.targets :
					def.aTargets;
	...
	function _fnGetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			 ...
			 */
			fetchData = function (data, type, src) {
				arrayNotation, funcNotation, out, innerSrc;
	...
	function _fnSetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			...
			setData = function (data, val, src) {
				a = _fnSplitObjNotation( src ), b;
				aLast = a[a.length-1];
				arrayNotation, funcNotation, o, innerSrc;
	...
	function _fnGetRowElements( settings, row, colIdx, d )
	{
		...
		attr = function ( str, td  ) {
			if ( typeof str === 'string' ) {
				idx = str.indexOf('@');
	
				if ( idx !== -1 ) {
					attr = str.substring( idx+1 );
					setter = _fnSetObjectDataFn( str );
		...
		cellProcess = function ( cell ) {
			if ( colIdx === undefined || colIdx === i ) {
				...
				if ( col && col._bAttrSrc ) {
					setter = _fnSetObjectDataFn( col.mData._ );
					setter( d, contents );
		...
		if ( td ) {
			...
			for ( j=0, jen=tds.length ; j<jen ; j++ ) {
				cellProcess( tds[j] );
			}
		}
	
		// Read the ID from the DOM if present
		rowNode = row.firstChild ? row : row.nTr;
	...
	{
		i, ien, cell, row, column;
		thead = oSettings.nTHead;
		tfoot = oSettings.nTFoot;
		createHeader = $('th, td', thead).length === 0;
	...
	{
		i, iLen, j, jLen, k, kLen, n, nLocalTr;
		aoLocal = [];
		aApplied = [];
	...
	function _fnDraw( oSettings )
	{
		...
		aPreDraw = _fnCallbackFire( oSettings, 'aoPreDrawCallback', 'preDraw', [oSettings] );
		...
		bServerSide = _fnDataSource( oSettings ) == 'ssp';
		...
		{
			iStart = bServerSide ? 0 : iDisplayStart;
			iEnd = bServerSide ? oSettings.aoData.length : iDisplayEnd;
	
			for ( j=iStart ; j<iEnd ; j++ )
			{
				iDataIndex = aiDisplay[j];
				...
				if ( iStripes !== 0 )
				{
					sStripe = asStripeClasses[ iRowCount % iStripes ];
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		...
		holding = $('<div/>').insertBefore( table ); // Holding element for speed
		features = oSettings.oFeatures;
		...
		aDom = oSettings.sDom.split('');
		featureNode, cOption, nNewNode, cNext, sAttr, j;
		for ( i=0 ; i<aDom.length ; i++ )
		{
			...
			{
				/* Plug-in features */
				aoFeatures = DataTable.ext.feature;
				for ( k=0, kLen=aoFeatures.length ; k<kLen ; k++ )
				{
			...
			if ( featureNode )
			{
				aanFeatures = oSettings.aanFeatures;
	...
	function _fnDetectHeader ( aLayout, nThead )
	{
		...
		nTr, nCell;
		i, k, l, iLen, jLen, iColShifted, iColumn, iColspan, iRowspan;
		bUnique;
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		if ( $.isPlainObject( ajax ) && ajax.data )
		{
			...
			newData = typeof ajaxData === 'function' ?
		...
		baseAjax = {
			...
			"error": function (xhr, error, thrown) {
				ret = _fnCallbackFire( oSettings, null, 'xhr', [oSettings, null, oSettings.jqXHR] );
	...
	function _fnAjaxParameters( settings )
	{
		...
		legacy = DataTable.ext.legacy.ajax;
		if ( legacy === null ) {
	...
	{
		dataSrc = $.isPlainObject( oSettings.ajax ) && oSettings.ajax.dataSrc !== undefined ?
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		searchFn = function() {
			...
			n = features.f;
			val = !this.value ? "" : this.value; // mental IE8 fix :-(
		...
		searchDelay = settings.searchDelay !== null ?
	...
	{
		oPrevSearch = oSettings.oPreviousSearch;
		...
		if ( _fnDataSource( oSettings ) != 'ssp' )
		{
			...
			for ( i=0 ; i<aoPrevSearch.length ; i++ )
			{
	...
	function _fnFilterCustom( settings )
	{
		...
		for ( i=0, ien=filters.length ; i<ien ; i++ ) {
			rows = [];
	
			// Loop over each row and see if it should be included
			for ( j=0, jen=displayRows.length ; j<jen ; j++ ) {
	...
	function _fnFilterCreateSearch( search, regex, smart, caseInsensitive )
	{
		...
		if ( smart ) {
			 ...
			 */
			a = $.map( search.match( /"[^"]+"|[^ ]+/g ) || [''], function ( word ) {
				if ( word.charAt(0) === '"' ) {
					m = word.match( /^"(.*)"$/ );
					word = m ? m[1] : word;
				}
	...
	function _fnFilterData ( settings )
	{
		columns = settings.aoColumns;
		column;
		i, j, ien, jen, filterData, cellData, row;
	...
	{
		i, iLen, iAjaxStart=settings.iInitDisplayStart;
		columns = settings.aoColumns, column;
		features = settings.oFeatures;
		deferLoading = settings.bDeferLoading; // value modified by the draw
		...
		dataSrc = _fnDataSource( settings );
		if ( dataSrc != 'ssp' || deferLoading ) {
			// if there is an ajax source load the data
			if ( dataSrc == 'ajax' ) {
				_fnBuildAjax( settings, [], function(json) {
					aData = _fnAjaxDataSrc( settings, json );
	...
	function _fnFeatureHtmlLength ( settings )
	{
		...
		for ( i=0, ien=lengths.length ; i<ien ; i++ ) {
		...
		div = $('<div><label/></div>').addClass( classes.sLength );
	...
	function _fnPageChange ( settings, action, redraw )
	{
		...
		changed = settings._iDisplayStart !== start;
	...
	function _fnFeatureHtmlTable ( settings )
	{
		...
		headerClone = $( table[0].cloneNode(false) );
		footerClone = $( table[0].cloneNode(false) );
	...
	function _fnScrollDraw ( settings )
	{
		...
		bScrolling = table.height() > divBodyEl.clientHeight || divBody.css('overflow-y') == "scroll";
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		if ( ie67 || ! userInputs && ! scrollX && ! scrollY &&
			...
			for ( i=0 ; i<columnCount ; i++ ) {
				colIdx = _fnVisibleToColumnIndex( oSettings, i );
		...
		else
		{
			...
			// table widths
			tmpTable = $(table).clone() // don't use cloneNode - IE8 will remove events on the main table
				.css( 'visibility', 'hidden' )
				.removeAttr( 'id' );
	
			// Clean up the table body
			tmpTable.find('tbody tr').remove();
			tr = $('<tr/>').appendTo( tmpTable.find('tbody') );
			...
			// needed
			holder = $('<div/>').css( scrollX || scrollY ?
					{
						position: 'absolute',
						top: 0,
						left: 0,
	...
	{
		idx = _fnGetMaxLenString( settings, colIdx );
		if ( idx < 0 ) {
			return null;
		}
	
		data = settings.aoData[ idx ];
	...
	function _fnSortListener ( settings, colIdx, append, callback )
	{
		...
		if ( append && settings.oFeatures.bSortMulti ) {
			...
			sortIdx = $.inArray( colIdx, _pluck(sorting, '0') );
	...
	function _fnSortData( settings, idx )
	{
		...
		// Use / populate cache
		row, cellData;
		formatter = DataTable.ext.type.order[ column.sType+"-pre" ];
	...
	function _fnLoadState ( settings, oInit, callback )
	{
		...
		loaded = function ( s ) {
			...
			abStateLoad = _fnCallbackFire( settings, 'aoStateLoadParams', 'stateLoadParams', [settings, s] );
			...
			// Reject old data
			duration = settings.iStateDuration;
	 ...
	 *   api = new $.fn.DataTable.Api( 'table.dataTable' );
	 */
	...
	$.extend( _Api.prototype, {
		...
		{
			for ( i=(start || 0), ien=this.length ; i<ien ; i++ ) {
				if ( this[i] === obj ) {
					return i;
				}
			}
		...
		iterator: function ( flatten, type, fn, alwaysNew ) {
			...
			if ( a.length || alwaysNew ) {
				api = new _Api( context, flatten ? a.concat.apply( [], a ) : a );
	...
	{
		if ( $.isArray( name ) ) {
			for ( j=0, jen=name.length ; j<jen ; j++ ) {
		...
		find = function ( src, name ) {
			for ( i=0, ien=src.length ; i<ien ; i++ ) {
				if ( src[i].name === name ) {
					return src[i];
				}
			}
	...
	_selector_first = function ( inst )
	{
		...
		for ( i=0, ien=inst.length ; i<ien ; i++ ) {
			if ( inst[i].length > 0 ) {
	...
	__row_selector = function ( settings, selector, opts )
	{
		...
		run = function ( sel ) {
			...
			if ( typeof sel === 'string' && sel.charAt(0) === '#' ) {
				...
				rowObj = settings.aIds[ sel.replace( /^#/, '' ) ];
				if ( rowObj !== undefined ) {
					return [ rowObj.idx ];
				}
	...
	_api_register( 'rows()', function ( selector, opts ) {
		...
		inst = this.iterator( 'table', function ( settings ) {
	...
	_api_registerPlural( 'rows().ids()', 'row().id()', function ( hash ) {
		...
		for ( i=0, ien=context.length ; i<ien ; i++ ) {
			for ( j=0, jen=this[i].length ; j<jen ; j++ ) {
				id = context[i].rowIdFn( context[i].aoData[ this[i][j] ]._aData );
				a.push( (hash === true ? '#' : '' )+ id );
			}
		}
	...
	_api_registerPlural( 'rows().remove()', 'row().remove()', function () {
		...
		this.iterator( 'row', function ( settings, row, thatIdx ) {
			...
			id = settings.rowIdFn( rowData._aData );
			if ( id !== undefined ) {
	...
	_api_register( 'rows.add()', function ( rows ) {
		newRows = this.iterator( 'table', function ( settings ) {
				row, i, ien;
				out = [];
	...
	_api_register( 'row.add()', function ( row ) {
		...
		rows = this.iterator( 'table', function ( settings ) {
			if ( row.nodeName && row.nodeName.toUpperCase() === 'TR' ) {
	...
	__details_remove = function ( api, idx )
	{
		...
		if ( ctx.length ) {
			row = ctx[0].aoData[ idx !== undefined ? idx : api[0] ];
	...
	__details_events = function ( settings )
	{
		...
		if ( _pluck( data, '_details' ).length > 0 ) {
			...
			api.on( colvisEvent, function ( e, ctx, idx, vis ) {
				...
				// a colspan)
				row, visible = _fnVisbleColumns( ctx );
	...
	__columnData = function ( settings, column, r1, r2, rows ) {
		a = [];
		for ( row=0, ien=rows.length ; row<ien ; row++ ) {
				...
				switch( match[2] ) {
					...
					case 'visible':
						...
						if ( idx < 0 ) {
							...
							visColumns = $.map( columns, function (col,i) {
	...
	__setColumnVis = function ( settings, column, vis ) {
		...
		if ( vis ) {
			...
			insertBefore = $.inArray( true, _pluck(cols, 'bVisible'), column+1 );
	...
	_api_registerPlural( 'columns().visible()', 'column().visible()', function ( vis, calc ) {
		ret = this.iterator( 'column', function ( settings, column ) {
			if ( vis === undefined ) {
	...
	_api_register( 'cells()', function ( rowSelector, columnSelector, opts ) {
		...
		// Row + column selector
		columns = this.columns( columnSelector );
	...
	_api_registerPlural(
		...
		function ( input, regex, smart, caseInsen ) {
			return this.iterator( 'column', function ( settings, column ) {
				preSearch = settings.aoPreSearchCols;
	...
	DataTable.isDataTable = DataTable.fnIsDataTable = function ( table )
	{
		...
		$.each( DataTable.settings, function (i, o) {
			head = o.nScrollHead ? $('table', o.nScrollHead)[0] : null;
	...
	DataTable.tables = DataTable.fnTables = function ( visible )
	{
		...
		a = $.map( DataTable.settings, function (o) {
	...
	_api_register( 'destroy()', function ( remove ) {
		...
		return this.iterator( 'table', function ( settings ) {
			...
			rows      = $.map( settings.aoData, function (r) { return r.nTr; } );
			i, ien;
			...
			removedMethod = remove ? 'remove' : 'detach';
			...
			idx = $.inArray( settings, DataTable.settings );
			if ( idx !== -1 ) {
	...
	DataTable.defaults = {
		 ...
		 *         for ( i=0, ien=json.length ; i<ien ; i++ ) {
	...
	DataTable.ext = _ext = {
		 ...
		 *        max = document.getElementById('max').value * 1;
	...
	$.extend( DataTable.ext.type.detect, [
		...
		function ( d, settings )
		{
			decimal = settings.oLanguage.sDecimal;
	...
	{
		return function() {
			args = [_fnSettingsFromNode( this[DataTable.ext.iApiIndex] )].concat(
	 ...
	 *     $('#table').dataTable().on('xhr.dt', function (e, settings, json) {
	 *       for ( i=0, ien=json.aaData.length ; i<ien ; i++ ) {
